...
async function deployViews() {
  try {
    ...
    // Deploy v_silver_pb_features with ALL 58 fields
    console.log('[1/2] Deploying v_silver_pb_features with 58 custom fields...');
    ...
    `);
    console.log('✅ v_silver_pb_features deployed with 58 fields');

    // Deploy v_silver_pb_subfeatures (no custom fields available yet)
    console.log('[2/2] Deploying v_silver_pb_subfeatures...');
    await conn.request().query(`
    ...
    const colResult = await conn.request().query(
      "SELECT COUNT(*) as col_count FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'v_silver_pb_features'"
    );
    console.log(`\n[v_silver_pb_features] Total columns: ${colResult.recordset[0].col_count}`);
