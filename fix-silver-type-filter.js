...
async function fixSilverTypeFilter() {
  try {
    ...
    console.log('[Updating v_silver_pb_features - Filter to type="feature" only]');
    await conn.request().query(`
      ...
      WHERE f.[type] = 'feature' OR f.[type] IS NULL
      GROUP BY f.id;
    `);
    ...
    const count = await conn.request().query(
      "SELECT COUNT(DISTINCT feature_id) as unique_features, SUM(CASE WHEN [Archived] = 'false' THEN 1 ELSE 0 END) as not_archived, SUM(CASE WHEN [Archived] = 'true' THEN 1 ELSE 0 END) as archived FROM [dbo].[v_silver_pb_features]"
    );

    console.log('\nVerification:');
    console.table(count.recordset);
    console.log('\nExpected: 2723 (false) + 682 (true) = 3405');
    console.log('Actual: ' + count.recordset[0].not_archived + ' (false) + ' + count.recordset[0].archived + ' (true) = ' + count.recordset[0].unique_features);
