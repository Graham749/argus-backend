  ...
  try {
    const [summaryRows, accountRows, hierarchyRows, dupRows, pbOnlyRows, zdUserRows, zdTicketRows, pbNoteRows, sfSubRows, phCovRows, phRows, phUnmatchedRows, mdmDomainRows, amRows] = await Promise.all([
      query(`
        SELECT
          ...
          COUNT(DISTINCT CASE WHEN pb_match_method = 'website_domain'  THEN pb_company_id END) AS pbWebsiteFallback,
          0                                                                                      AS pbName,
    ...
    const payload = {
      summary: {
        ...
        pbWebsiteFallback: Number(summary.pbWebsiteFallback) || 0,
