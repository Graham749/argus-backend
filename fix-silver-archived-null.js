...
async function fixSilverArchivedNull() {
  try {
    ...
    console.log('[Updating v_silver_pb_features - Filter OUT NULL archived]');
    await conn.request().query(`
      CREATE OR ALTER VIEW [dbo].[v_silver_pb_features] AS
      SELECT
        f.id AS feature_id,
        MAX(f.name) AS feature_name,
        CASE WHEN MAX(CAST(f.archived AS INT)) = 1 THEN 'true' WHEN MAX(CAST(f.archived AS INT)) = 0 THEN 'false' ELSE NULL END AS [Archived],
      ...
      GROUP BY f.id;
    `);
    console.log('✅ v_silver_pb_features updated');

    // Verify
    const count = await conn.request().query(
      'SELECT COUNT(DISTINCT feature_id) as unique_features, COUNT(*) as total_rows FROM [dbo].[v_silver_pb_features]'
    );
    console.log(`\nDistinct features: ${count.recordset[0].unique_features}, Total rows: ${count.recordset[0].total_rows}`);

    const archived = await conn.request().query(`
      SELECT
        CASE WHEN [Archived] LIKE '%false%' THEN 'false' WHEN [Archived] LIKE '%true%' THEN 'true' ELSE '[Other]' END as status,
        COUNT(*) as count
    ...
    `);
    console.log('\nArchived breakdown:');
    archived.recordset.forEach(r => console.log(`  ${r.status}: ${r.count}`));
